    ...
    <main id="main-content" tabIndex={-1} className="mx-auto max-w-4xl space-y-6 p-4 sm:p-6 focus:outline-none">
      ...
      {review && <>
        ...
        {review.complete && <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-card-soft sm:p-8">
          <div className="flex items-start gap-3"><ExternalLink className="mt-1 h-5 w-5 text-brand-700" aria-hidden="true" /><div><h2 className="text-lg font-bold">Browser automation</h2><p className="mt-1 text-sm text-slate-600">AccessAI analyzes the target website in the cloud to check form compatibility and map fields from your document. You will see a detailed field-mapping report below.</p></div></div>
          ...
          {automation && <div className="mt-4 space-y-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-950" aria-live="polite"><p className="font-bold">Form compatibility analysis completed in the cloud</p><p>AccessAI mapped {automation.filled.length} high-confidence field{automation.filled.length === 1 ? '' : 's'}. Nothing has been submitted.</p>{automation.filled.length > 0 && <ul className="list-disc pl-5">{automation.filled.map((field) => <li key={field.sourceField}>{field.sourceField} → {field.targetField}</li>)}</ul>}{automation.manualReview.length > 0 && <div><p className="font-semibold">Manual review required</p><ul className="list-disc pl-5">{automation.manualReview.map((field) => <li key={field.sourceField}>{field.sourceField}: {field.reason}</li>)}</ul></div>}</div>}
          <div className="mt-5"><Button onClick={() => void automateForm()} disabled={automating || !targetUrl.trim()} leftIcon={<ExternalLink className="h-4 w-4" />}>{automating ? 'Analyzing website form…' : 'Analyze target website form compatibility'}</Button></div>
        </section>}
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-card-soft sm:p-8">
